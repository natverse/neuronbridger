---
title: "From EM neurons to peripheral targets via GAL4 lines"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{From EM neurons to peripheral targets via GAL4 lines}
...
Generate hypotheses linking VNC efferent neurons to specific peripheral
tissues by chaining three resources:

1. **[MANC](https://www.janelia.org/project-team/flyem/manc-connectome)**
   (Male Adult Nerve Cord) connectome via the
   [`malevnc`](https://github.com/natverse/malevnc) R package — pick up
   abdominal efferents:
    - **`MNad*`** abdominal motor neurons;
    - **`EN*`** efferent / endocrine neurons with soma in an abdominal
      neuromere (`A1`–`A8`). Note: the prefix is `EN`, not `ENN`.
2. **[NeuronBridge](https://neuronbridge.janelia.org/)** colour-depth
   MIP search (data release `v3_9_0`, the first to ship MANC MIPs) —
   find GAL4 / split-GAL4 lines that label each EM cell.
3. **[KDRC KGutProject](https://flyinfo.kr/FlexForm_KGutProject_Load.do?tmpl=template/E)**
   ([Lim et al. 2021](https://doi.org/10.1080/01677063.2020.1853722)) —
   for any matched line, look up which adult-gut region it stains. A
   shared line is strong indirect evidence that the EM cell innervates
   that region.

Very few VNC efferents have a confirmed peripheral target, so the
output is a ranked list of *candidates* for follow-up experiments.

> **Template space.** This vignette uses NeuronBridge's pre-computed
> VNC MIP libraries — it doesn't render new MIPs from MANC meshes.
> If you want to render MIPs to compare directly, the VNC space is
> `JRC2018VNCU_HR` (dims `573 × 1119 × 219`, voxdims
> `0.461 × 0.461 × 0.7` µm) — the VNC analogue of `JRC2018U_HR` used
> by [*Render a MIP from an EM neuron*](mip_from_em.html) and
> [*Render a MIP from registered LM data*](mip_from_lm.html). Pass
> `target_space = "VNC"` to `nrrd_to_mip()`.
