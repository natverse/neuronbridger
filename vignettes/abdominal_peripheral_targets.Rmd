...
> **Template space note.** This vignette runs against
> NeuronBridge's pre-computed VNC ColorMIP libraries — it doesn't
> render new MIPs from MANC meshes. Should you want to render MIPs
> from MANC neurons (e.g. to compare against the FlyLight VNC hit),
> the NeuronBridge VNC space is **`JRC2018VNCU_HR`** (a.k.a.
> `JRC2018_VNC_UNISEX_461`, dims `573 × 1119 × 219`, voxdims
> `0.461 × 0.461 × 0.7` µm) — the VNC analogue of the brain
> `JRC2018U_HR` used by the
> [*colour-MIPs from a connectome neuron*](colormip_direct_vs_fiji.html)
> and [*colour-MIPs from a registered LM volume*](colormip_lm.html)
> vignettes. Pass `target_space = "VNC"` to `nrrd_to_mip()` to render
> into that grid.
